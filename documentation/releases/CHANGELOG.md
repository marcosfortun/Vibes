...
- **Imagen también para resultados de IA y altas "solo título"** (`resolveImage`): al crear (alta simple y carga masiva), si el candidato no trae imagen se resuelve en cascada best-effort con timeout corto — fuente especializada por categoría primero (**TMDB** cine/series/documental, **Steam** videojuegos/VR, **iTunes Search** podcasts y música, **BoardGameGeek** juegos de mesa; todas sin API key nueva) y **Wikipedia** (pageimages, wiki del idioma con fallback a la inglesa) como red universal para el resto (museos, rutas, festivales…). La IA ahora devuelve `wikiTitle` (título exacto del artículo) para afinar la búsqueda sin inventar URLs. Si nada responde, la recomendación se crea sin imagen. *(Nota: la especializada va antes que Wikipedia — desviación del orden propuesto — porque da la imagen "de ficha": en pruebas reales, Wikipedia devolvía la foto del presentador de Radiolab en vez de la carátula.)*
